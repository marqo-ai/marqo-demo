...
export const SearchBar = () => {
  ...
  return (
    <Fragment>
      <div
        className={`flex w-full h-[50px] mb-20 rounded-lg ${
          theme === 'dark' ? 'bg-slate-700' : 'searchBar'
        }`}
      >
        <div className="searchInputWrapper border-[#00FFAA] flex h-full w-full justify-left self-center rounded-sm py-0 pr-0 pl-2">
          <input
            ...
            placeholder="Search..."
            className={`bg-transparent ml-2 px-0 w-full text-sm md:text-md focus:outline-none ${
        ...
        <Button
          ...
          className={`bg-primary h-full rounded-r-lg rounded-l-none text-white hidden sm:block hover:border-none hover:bg-secondary hover:text-primary ${
            theme === 'dark' && 'bg-secondary text-primary border-none'
          } ${disableClasses}`}
        >
          Search
        </Button>
        <div className="sm:hidden absolute w-full mt-[150px] flex justify-center">
          <Button
            ...
            color="primary"
            disabled={!q}
            className={`w-[200px] rounded-lg text-white hover:bg-secondary hover:text-primary hover:border-none ${
