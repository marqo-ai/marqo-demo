...
export const UploadImg = () => {
    ...
    const handleOnChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (e?.target?.files && e?.target?.files.length > 0) {
            if (ACCEPTED_FILE_TYPES.includes(e?.target?.files[0].type) && e.target.files[0].size > 0 && getSizeInMegabytes(e.target.files[0].size) <= MAX_FILE_SIZE_IN_MEGABYTES) {
            ...
            } else if (e.target.files[0].size < 1 || getSizeInMegabytes(e.target.files[0].size) > MAX_FILE_SIZE_IN_MEGABYTES) {
        ...
        <div className="contents">
            <div className="flex items-center justify-center w-full">
                <label htmlFor="img-file" className="flex flex-col items-center justify-center w-[fit-content] cursor-pointer">
                    ...
                    <Input onChange={(e) => handleOnChange(e)} type={"file"} id="img-file" className={`hidden`} accept="image/png, image/jpeg, image/jpg" />
                </label>
            </div>
        </div>
    </Fragment>
